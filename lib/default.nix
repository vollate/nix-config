...
rec {
  ...
  mkSystem = { hostname, system ? "x86_64-linux", username, modules ? [ ]
    , extraModules ? [ ], desktop ? null, overlays ? [ ], specialArgs ? { }, }:
    inputs.nixpkgs.lib.nixosSystem {
      inherit system;

      specialArgs =
        ({ inherit inputs hostname username desktop; } // specialArgs);
  ...
  mkHost = args:
    mkSystem (args // {
      modules = [ ../modules/base ../modules/desktop ../modules/development ]
        ++ (args.modules or [ ]);
    });
}
